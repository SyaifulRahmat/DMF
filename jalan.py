...
CON=sol()
#------------------[ USER-AGENT ]-------------------#
ugen=['Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36']
ugen2=['Mobile/15E148 Safari/604.1 Mozilla/5.0 (iPhone; CPU iPhone OS 15_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/15.6 Mobile/15E148 Safari/604.1']
cokbrut=[]
...
def login_lagi334():
	try:
		...
		cookie=input(f'\033[1;31;1m  [•] Paste Cookies :{asu} ')
		...
		print(f'  {x}[{h}•{x}]{h} Login Suskes...Jalankan Lagi Perintahnya !{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s Login Eror Cookies Invalid !!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
    ...
    print('')
	alvino_xy(f' Informasi ID   : '+str(my_id))
	alvino_xy(f' Proxy Sim Card : {ip}{x}')
	print('')
	...
	if alok in ['1']:
		...
		print('<\> Log-out Suskes Terima Kasih')
		exit()
	else:
...
def result():
	print('')
	print('\33[m[1] \033[33mHasil Crack CP ')
	print('\33[m[2] \x1b[1;92mHasil Crack OK ')
	print('\33[m[0] \033[1;31;1mKembali	')
	kz = input('\n\33[m<\> Pilih : ')
	if kz in ['1','01']:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('[!] Dump berapa Target ? : '))
	except ValueError:
	...
	try:
		print('')
		print(f'[!] Total Target Dump {h}'+str(len(id)))
		time.sleep(2)		
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
